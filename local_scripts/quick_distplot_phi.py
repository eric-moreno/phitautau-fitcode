...
parser.add_argument('--prefix',      dest='prefix',      default="topCR",             help="control or signal region")
parser.add_argument('--channel',      dest='channel',      default="hadel",             help="channel hadhad, hadel, or hadmu")
...
for second_dir in args.regions:
    ...
    print(h_err, h_top, h_wlnu, h_ztt, h_vvqq, h_qcd, h_h125, h_sig, h_dat)
    ...
    if not os.path.exists(os.path.join(args.prefix, args.channel)):
    # If it doesn't exist, then create it
        os.mkdir(os.path.join(args.prefix, args.channel))

    canvas.SaveAs('%s/%s/fitplot_%s%s.pdf'%(args.prefix, args.channel, second_dir, args.label))
    pad1.SetLogy()

    canvas.SaveAs('%s/%s/fitplot_%s%s_logy.pdf'%(args.prefix, args.channel, second_dir,args.label))
    pad1.SetLogy(0)
